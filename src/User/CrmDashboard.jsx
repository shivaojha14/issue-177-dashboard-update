...
function CrmDashboard() {
  ...
  const [customerCountByStatus, setCustomerCountByStatus] = useState({});
  const [loading, setLoading] = useState(true);
  const [entriesPerPage, setEntriesPerPage] = useState(10); // Initial value of entries per page
  const [totalCustomerInCurrentMonth, setTotalCustomerInCurrentMonth] =useState({});
  const [totalCustomerAddedByMonthInCurrentYear,setTotalCustomerAddedByMonthInCurrentYear] = useState({});
  const [convertedCustomer, setConvertedCustomer] = useState(null);
  const [totalCustomer, setTotalCustomer] = useState(null);
  const [activeCustomer, setActiveCustomer] = useState(null);
  const [inactiveCustomer, setInactiveCustomer] = useState(null);
  let userData = [
    {
      id: 1,
      year: 2021,
  ...
  const fetchYearlyBarChartData = async (year) => {
    try {
      ...
      const response = await axios.get(`${path_url}/customer/dashboard/${adminId}`, {
        headers: {
      ...
      const data = response.data.body.totalCustomerAddedByMonthInCurrentYear;
  ...
  const fetchMonthlyBarChartData = async (selectedMonth) => {
    try {
      ...
      const response = await axios.get(
        `${path_url}/customer/dashboard/${adminId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      const data = response.data.body.totalCustomerInCurrentMonth;
    ...
    if (token) {
      const fetchTable = async () => {
        try {
          ...
          setEntriesPerPage(
            data.customerList.length % 2 === 0
              ? data.customerList.length / 2
              : Math.ceil(data.customerList.length / 2)
          );
          setCustomerList(data.customerList);
          setCustomerCountByStatus(data.customerCountByStatus);
          setTotalCustomerInCurrentMonth(data.totalCustomerInCurrentMonth);
          setTotalCustomerAddedByMonthInCurrentYear(
            data.totalCustomerAddedByMonthInCurrentYear
          );
          setLoading(false);
          setConvertedCustomer(data.convertedCustomer);
          setActiveCustomer(data.activeCustomer);
          setTotalCustomer(data.totalCustomer);
          setInactiveCustomer(data.inActiveCustomer);
  ...
  }, [selectedMonth],[selectedYear]); // Added selectedMonth and year as dependency
  ...
  const statusColors = {
    ...
    CONVERTTOCUSTOMER: "rgba(0, 0, 255, 0.8)",
  ...
  const chartData = {
    labels: Object.keys(customerCountByStatus),
    datasets: [
      {
        data: Object.values(customerCountByStatus),
  ...
  const lineChartData = {
    ...
    datasets: [
      {
        label: "Total Customers in Current Month",
        data: Array.from(
          { length: 31 },
          (_, i) => totalCustomerInCurrentMonth[padNumber(i + 1)] || 0
        ),
        backgroundColor: [
          "rgba(22, 144, 0, 0.50)",
          "rgba(7, 181, 249, 0.50)",
  ...
  return (
    <>
      ...
      <div className="greet">
        ...
        >
          <div className="col-sm-4 mt-2 ml-2 mb-2 subcon6">
            ...
            <div className=" mt-2">Welcome to the Customer Dashboard.</div>
          </div>
      ...
      <div className="col-12 d-flex justify-content-center">
        <div
          className="col w-100 d-flex justify-content-center pt-1 mb-4 border rounded position-relative"
          style={{
            width: "100%",
            height: "350px",
            paddingLeft: 10,
        ...
        >
          <div className="title">
            ...
            >
              ...
              Customer Added In Current Month
            </label>
          </div>
          <div
